...
func idExpression(
	...
	return table.Withdraws.ID.EQ(pgjet.Int(id.ID()))
}

func idsExpression(
	ids []balance.BalanceID,
) pgjet.BoolExpression {
	exp := make([]pgjet.Expression, 0)
	for _, id := range ids {
		exp = append(exp, pgjet.Int(id.ID()))
	}
	return table.Withdraws.ID.IN(exp...)
}

func userIDExpression(
	id common.UserID,
) pgjet.BoolExpression {
	return table.Withdraws.UserID.EQ(pgjet.Int(id.ID()))
}
...
func userIDsExpression(
	...
	return table.Withdraws.UserID.IN(exp...)
}
